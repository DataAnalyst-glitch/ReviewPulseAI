...
## 10. Update Brief — Pandas Statistical Layer

> Given after Section 9 shipped. A lightweight pandas-based complement to the LLM analysis, not a replacement: (1) a rating-vs-sentiment accuracy cross-check, (2) a statistical comparison table (avg rating/review count/sentiment ratio) alongside the AI-generated gap opportunities, (3) a pain-point-mention-frequency-over-time chart. All three explicitly required to stay clearly labeled as statistical/rating-based, distinct from the AI-generated insights.

**Status: implemented and verified live** (`src/analysis/stats.py`, new module, pure pandas, no LLM calls anywhere in it):

- **Sentiment mismatch guardrail** (`find_sentiment_mismatches`) — flags 4-5 star reviews the LLM classified Negative, or 1-2 star reviews classified Positive, as "Sentiment mismatch - needs review." A second, independent accuracy guardrail alongside the existing quote-verification check in `guardrail.py` — that one checks Agent B's quotes against source text; this one checks Agent A's sentiment against the reviewer's own star rating. Required persisting `review_date` (and reusing already-stored `rating`) per review in a new `sentiment_results.review_date` column (same self-healing migration pattern as prior additions) plus a new `get_sentiment_details()` storage query for per-review rows.
- **Statistical comparison table** (`build_comparison_table`) — genuine pandas `groupby` across all products' review-level rows (not just reused pre-aggregated counts), producing review count / avg rating / sentiment % per product, main-first order preserved. Shown in Streamlit and the PDF right alongside (not instead of) the AI-generated Feature Gap Opportunities, both sections explicitly labeled so a reader knows which claim is statistical vs. AI-interpreted.
- **Review Timeline** (`build_pain_point_timeline`, weekly `pd.to_period` buckets using each pain point's `supporting_review_ids` cross-referenced against review dates) — a proxy signal since sales/demographic data isn't accessible from reviews. Gracefully empty (with an explanatory note, not a misleading blank chart) when there's under 2 weeks of date coverage. Streamlit renders the full chart (plotly, fixed categorical color order reused from the existing stat-card palette); the PDF renders a compact earlier-half-vs-later-half trend table instead (`summarize_timeline_trend`) rather than embedding a chart image, consistent with the existing "no `kaleido` dependency" PDF design decision.

Caught one real bug via the pypdf text-extraction test (not the visual PDF preview, which had misleading rendering artifacts for the new multi-column table rows that weren't present in the actual PDF content stream — pypdf's extracted text was the authoritative check, per the project's own established pattern for verifying fpdf2 output). All three additions verified live end-to-end in-browser (chart renders, tables populate, accuracy-check success/warning states both exercised) with no console errors.
