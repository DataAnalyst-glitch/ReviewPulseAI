...
- **"Listen to Summary" (Voice Output, `CLAUDE.md` Section 7 Addition 2, tuned to a 30-45s spec in Section 9 Addition 4)** — a template-built 3-point spoken summary (sentiment split, #1 pain point, #1 recommendation; `src/report/voice_summary.py`, no LLM call — the numbers are already known, this is formatting) played via the browser's native `speechSynthesis` (Web Speech API, zero cost, zero new Python dependency). The summary text is always shown visibly above the button — never audio-only — and the same text is a fallback "Executive Summary" section at the top of the PDF, so the report doesn't depend on audio playback. Feature-detected client-side: browsers without `speechSynthesis` get a message instead of a silent failure, and the rest of the app works identically either way.

**Trust/accuracy disclaimer** — a short, professional note ("This analysis is based on customer review text only... does not incorporate sales volume, pricing, or market trend data...") shown as a caption under the Streamlit results header and in the PDF's footer on every page, from one shared `disclaimer_text()` in `src/report/__init__.py` so the two surfaces can't drift apart.

**Statistical layer (`src/analysis/stats.py`, pandas only, no LLM call)** — a lightweight complement to the LLM analysis, not a replacement, and labeled "Statistical (pandas)" everywhere it's shown so a reader can always tell which claim came from which method:

- **Statistical Accuracy Check** — cross-checks each review's star rating against Agent A's sentiment label (`find_sentiment_mismatches`), flagging 4-5 star reviews classified Negative or 1-2 star reviews classified Positive. A second, independent guardrail alongside the existing quote-verification check in `src/analysis/guardrail.py` (that one checks Agent B's pain-point quotes against source text; this one checks Agent A's sentiment against the reviewer's own rating). Skips gracefully with an explanatory note if the product's reviews have no rating data.
- **Statistical Comparison table** — average rating, review count, and sentiment ratio side by side for the main product vs. each competitor (`build_comparison_table`, a real pandas `groupby` across all products' review-level rows), shown alongside the AI-generated Feature Gap Opportunities as "hard numbers" support, not a replacement for them.
- **Review Timeline** — pain-point mention frequency over time (`build_pain_point_timeline`, weekly `pd.to_period` buckets), a proxy signal for when an issue may be affecting sales since exact sales/demographic data isn't accessible from reviews alone. Reviews with missing/unparseable dates are excluded rather than blocking the rest of the report; the whole section is skipped with a note if there isn't enough date coverage (fewer than 2 distinct weeks). Streamlit shows the full weekly chart; the PDF (which deliberately doesn't embed chart images — see the PDF export note above) shows a compact earlier-half-vs-later-half trend table instead, built from the same timeline data (`summarize_timeline_trend`).
...
**Access-code unlock (`ACCESS_CODES` env var):** so a paying client can run their own order directly on the public deployment instead of the operator running it locally for them. When `DEMO_MODE` is on, the sidebar shows a password-style "Have an access code?" field; entering a value listed in `ACCESS_CODES` (comma-separated, set in Streamlit Secrets, e.g. `ACCESS_CODES = "client1-code,client2-code"`) unlocks the product-ID and CSV-upload fields for that browser session only — closing the tab or opening a new session re-locks it, since the unlock lives in `st.session_state`, not anywhere persistent. No database or expiry system by design (documented tradeoff — add one if code reuse/rotation becomes a real problem). Leaving the field empty leaves the demo experience exactly as before. No effect when `DEMO_MODE` is unset (local full mode has nothing to unlock).
