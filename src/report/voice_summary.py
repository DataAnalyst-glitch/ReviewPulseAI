...
already loudly obvious from the dashboard. Worded to land in a natural
30-45 second spoken range (~90-110 words at a typical browser TTS rate)
without padding: the pain point's own description (already generated by
Agent B, not invented here) does the work of filling that out.
"""
...
def build_voice_summary(report: ComparisonReport) -> str:
    ...
    neutral_pct = main.sentiment_counts.get("Neutral", 0) / total * 100
    ...
    sentences = [
        ...
        f"Sentiment breaks down to {positive_pct:.0f} percent positive, "
        f"{neutral_pct:.0f} percent neutral, and {negative_pct:.0f} percent negative.",
    ]

    if main.pain_points:
        top = main.pain_points[0]
        sentences.append(f"Your number one customer complaint is {top['pain_point']}.")
        description = top.get("description")
        if description:
            sentences.append(description)
        action = top.get("recommended_action")
        if action and action != INSUFFICIENT_DATA:
            sentences.append(f"Our recommended action: {action}")
    else:
        sentences.append("No major pain points were identified in this batch of reviews.")

    sentences.append(
        "That's your headline for now - see the full dashboard and PDF report for every "
        "pain point and feature gap opportunity."
    )
